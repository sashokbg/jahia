...
<jcr:nodeProperty var="pagePicker" name="pagePicker" node="${currentNode}" />
<jcr:nodeProperty var="nbOfQuestions" name="nbOfQuestions" node="${currentNode}" />
<jcr:nodeProperty var="nbOfAnswers" name="nbOfAnswers" node="${currentNode}" />
...
<div id="surLesForums">
	...
	<c:if test="${nbOfQuestions.long > 0}">
		<%-- ******* Select all posts that have no replies and that are pickable ******* --%>
		<jcr:xpath
			xpath="/jcr:root${queryPath}//element(*, jnt:ciQuestion)
				[@nbOfReplies = 0 
					and @nbOfProfReplies = 0
					and @isNotPickable != 'true'
					and (@jcr:language = 'fr'
					or not(@jcr:language))]
				order by @jcr:created descending"
			limit="${nbOfQuestions.long}" var="listOfPost"/>
		...
		<c:choose>
			<c:when test="${listOfPost.nodes.size > 0}">
				<c:forEach items="${listOfPost.nodes}" var="post" varStatus="status">
					...
					<div
						class="ligneQuestionHome ${status.count % 2 == 0 ? 'pair' : 'impair'} nonAnswered">
						<div class="avatar">
	...
	<%-- ****** Select all replies  ****** --%>
		<jcr:xpath
			xpath="/jcr:root${queryPath}//element(*, jnt:ciReply)
				[@isNotPickable != 'true'
					and (@jcr:language = 'fr'
					or not(@jcr:language))]
				order by @jcr:created descending"
			limit="${nbOfAnswers.long}" var="listOfPost"/>
...
<utility:logger value="search XPATH module homeForumNews executed in ${time} ms" level="info" />