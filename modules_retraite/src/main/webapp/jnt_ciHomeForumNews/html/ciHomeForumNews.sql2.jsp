...
<div id="surLesForums">
	...
	<c:if test="${nbOfQuestions.long > 0}">
		...
		<jcr:sql
			sql="select * from 
				[jnt:ciQuestion] as q 
				where isdescendantnode(q, '${queryPath}')
				and q.[nbOfReplies]=0
				and q.[nbOfProfReplies]=0
				and q.[isNotPickable] <>true
				order by q.[jcr:created] desc"
			limit="${nbOfQuestions.long}" var="listOfPost"/>
			
		<c:if test="${listOfPost.nodes.size <= 0}">
		<jcr:sql
			sql="select * from [jnt:ciQuestion] as q
				where isdescendantnode(q, '${queryPath}')
				and q.[isNotPickable]<>true
				order by q.[jcr:created] desc"
			limit="${nbOfQuestions.long}" var="listOfPost"/>
	...
	<%-- ****** Select all replies  ****** --%>
		<jcr:sql
			sql="select * from [jnt:ciReply] as r
				where isdescendantnode(r, '${queryPath}')
				and r.[isNotPickable]<>true
				order by r.[jcr:created] desc"
...
<utility:logger value="search JCR/SQL2 module homeForumNews executed in ${time} ms" level="info" />