...
<h2 id="questions" class="titrePage titreThematiquePage">${empty title ? forumName : title.string}</h2>
...
<c:set var="maxItem">
<c:if test="${not empty renderContext.site.properties['paginationmaxQuestionList']}">${renderContext.site.properties['paginationmaxQuestionList'].long}</c:if>
<c:if test="${empty renderContext.site.properties['paginationmaxQuestionList']}">10</c:if>
</c:set>
	...
	<c:choose>
		<c:when test="${moduleMap.listTotalSize > 0}">
			...
			<div class="voirLes">
				...
				<form method="get" name="voirLes" action="#questions">
		...
		<c:otherwise>
			<div class="dataSection">Aucune question n'a encore &eacute;t&eacute; pos&eacute;e, <a href="${renderContext.site.properties['addQuestionLink'].node.url}?rubric=${cia:getRubricParent(currentNode).path}&thematic=${cia:getThematicParent(currentNode).path}" title="Posez votre question"  class="mbLink">posez une question</a> !</div>
		</c:otherwise>
	</c:choose>
</div>
