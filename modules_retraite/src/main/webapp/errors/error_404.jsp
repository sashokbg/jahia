...
<%@ taglib prefix="cia"
	uri="http://www.ag2rlamondiale.fr/tags/jahia/cia"%>

<cia:errorPage errorNumber="404" />

<c:choose>
	<c:when test="${not empty errorPage}">
		${errorPage}
	</c:when>
	<c:otherwise>
		...
		<html xmlns="http://www.w3.org/1999/xhtml">
			...
			<link rel="stylesheet"
				href="${request.contextPath}/css/admin-1.1.css"
				type="text/css" />
			...
			<body class="login"
				...
				<div id="adminLogin">
					...
					<br class="clearFloat" />
					<h3 class="loginIcon">
						<fmt:message key="label.error.404.title" />
					</h3>
					<p>
						<fmt:message key="label.error.404.description" />
					</p>
					...
					<p>
						<fmt:message key="label.error.homeLink" />
						:&nbsp;<a href="<c:url value='/'/>"><fmt:message
								key="label.homepage" /></a>
					</p>
					<br class="clearFloat" />
				</div>
			</body>
