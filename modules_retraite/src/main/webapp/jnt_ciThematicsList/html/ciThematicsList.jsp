<%--
	...
	@Id : jnt_ciThematicsList
	@description : Liste des thematiques
...
<c:when test="${empty rubricPage }">
	Ceci n'est pas un page de visualisation d'une th�matique.
</c:when>
<c:otherwise>
	<c:set var="thematicsList" value="${jcr:getChildrenOfType(rubricPage,'jnt:page')}" />
	...
	<c:forEach items="${thematicsList }" var="thematic" varStatus="status">
		...
		<div class="boiteRubrique ${divCssClass }">
			<h3 class="titreBoiteRubrique">
				<a href="${thematic.url }">${thematicTitle }</a>
			</h3>
