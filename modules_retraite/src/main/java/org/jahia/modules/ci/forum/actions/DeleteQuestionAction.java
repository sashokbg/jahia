...
public class DeleteQuestionAction extends Action implements CiConstants {
		...
		return (ActionResult) jcrTemplate.doExecuteWithSystemSession(user.getName(), session.getWorkspace().getName(), session.getLocale(),
				new JCRCallback<Object>() {
					public Object doInJCR(JCRSessionWrapper session) throws RepositoryException {
						...
						if (Validator.isNotNull(questionNode)) {
							ForumHelper.deletePostAndDecrementPostNumbersOf(questionNode);
							JCRTagUtils.getParentOfType(questionNode, "jnt:page").remove();
						}
