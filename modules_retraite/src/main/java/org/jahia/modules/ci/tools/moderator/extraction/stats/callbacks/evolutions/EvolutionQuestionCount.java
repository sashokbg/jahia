...
public class EvolutionQuestionCount extends AbstractStatCallback {

	private final transient static Logger LOGGER = org.slf4j.LoggerFactory.getLogger(EvolutionQuestionCount.class);
	...
	public Object getData(JCRSessionWrapper session, RenderContext context) {
		...
		String query = "select * from [jnt:ciQuestion]";
		...
		if (nodes != null) {
			while (nodes.hasNext()) {
				...
				maxDate = new Date();
				if ((AbstractExtractor.minDate.before(date) || AbstractExtractor.minDate.equals(date))
						&& (maxDate.after(date) || maxDate.equals(date))) {
					...
					if (LOGGER.isDebugEnabled())
						LOGGER.debug(simplifiedDate + " : " + node.getPropertyAsString("title") + "\t[" + questionDateMap.get(simplifiedDate)
								+ "]");
				}
			}
