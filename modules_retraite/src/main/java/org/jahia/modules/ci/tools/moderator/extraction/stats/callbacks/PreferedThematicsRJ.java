...
public class PreferedThematicsRJ extends AbstractStatCallback {
	private final transient static Logger LOGGER = org.slf4j.LoggerFactory.getLogger(PreferedThematicsRJ.class);
	...
	public Object getData(JCRSessionWrapper session, RenderContext renderContext) {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				if (user.hasProperty(PROPERTIES_USER_SELECTED_THEMATICS))
					userThematic = user.getProperty(PROPERTIES_USER_SELECTED_THEMATICS).getString();
				...
				if (StringUtils.isNotEmpty(userThematic)) {
					...
					for (String part : StringUtils.split(userThematic, delem)) {
						try {
							...
							JCRNodeWrapper rubricPage = JCRTagUtils.getParentOfType(node, "jnt:page");
							String thematic = getThematicFullName(node, rubricPage);
