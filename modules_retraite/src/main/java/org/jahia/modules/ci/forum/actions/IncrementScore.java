 ...
 *
 * @author : m.el-aarko
 * @description  : incrémenter le score d'une page question.
 */
			...
			new JCRCallback<Object>() {
	            public Object doInJCR(JCRSessionWrapper session) throws RepositoryException {
	            	...
	            	if (Validator.isNotEmpty(nodePath)) {
		            	...
		            	if(!node.isNodeType("jmix:ciRated")) {
		                    node.addMixin("jmix:ciRated");
		                    node.setProperty("score", 0);
		                    session.save();
		                }
		                // Update number of views
		                long nbOfViews = node.getProperty("score").getLong();
		                node.setProperty("score", ++nbOfViews);
		                session.save();
	            	}
